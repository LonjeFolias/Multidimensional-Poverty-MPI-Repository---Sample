********************************************************************************
    ...
    log   using  "$pathlog/01_cmdpi_2019", replace
	
    di in yellow "`c(current_date)' `c(current_time)'"

/*******************************************************************************
************    PART 1:  CHILD MULTI-DIMENSIONAL POVERTY INDEX     *************
************                 WAVE III (2019)		               *************
********************************************************************************

# Name:			01_Child-Multi-Dimensional Poverty Index 2019

# Purpose:		Generates Child Multi-Dimensional Poverrty Index for 2019 wave
			
# Creates: 		cmdpi-2019.dta
...
# Ado(s):	labutil, labutil2, mdesc (ssc install labutil, labutil2, mdesc)

*	Uses the following modules;

		(01) hh_mod_a_filt_19 - Household Identification
		(02) hh_mod_b_19	  - Household Roster
		(03) hh_mod_c_19	  - Education
		(04) hh_mod_d_19	  - Health
		(05) hh_mod_e_19	  - Time Use and Labour
		(06) hh_mod_f_19	  - Housing
		(07) hh_mod_h_19	  - Food Security
		(08) hh_mod_v_19	  - Child Anthropometry
		(09) hh_mod_l_19	  - Durable goods
		(10) hh_mod_g1_19     - Food Consumption Over Past One Week
...
use      "${wave3}/hh_mod_l_19.dta", clear  
...
collapse (max) inforasset, by(y4_hhid)
lab var  inforasset "HH has any of the information assets: radio, TV, PC, satelite dish"

* Merge Household Identification
merge    1:1 y4_hhid   using "${wave3}/hh_mod_a_filt_19.dta", nogen 
 
* Merge Household Roster
merge    1:m y4_hhid   using "${wave3}/hh_mod_b_19.dta", nogen  

* Merge Education
merge 1:1  y4_hhid PID using "${wave3}/hh_mod_c_19.dta", keep (3) nogen 

* Merge Health
merge 1:1  y4_hhid PID using "${wave3}/hh_mod_d_19.dta", keep (3) nogen 

* Merge Time Use and Labour
merge 1:1  y4_hhid PID using "${wave3}/hh_mod_e_19.dta", keep (3) nogen

* Merge Housing
merge m:1  y4_hhid     using "${wave3}/hh_mod_f_19.dta", keep (3) nogen

* Merge Food Security
merge m:1 y4_hhid  	   using "${wave3}/hh_mod_h_19.dta", keep (3) nogen

* Merge Child Anthropometry
merge 1:1  y4_hhid PID using "${wave3}/hh_mod_v_19.dta", keep (3) nogen
...
***	Definitions of indicators are adapted from the Child Multi-Dimensional
...
* ------------------------------------------------------------------------------
codebook hh_h03c
gen      ind_meals = hh_h03c < 3 if age_6_59m ==1
...
lab var dim_nutrition "Nutrition dimension - deprivation in at least one indicator"
********************************************************************************
...
tab1    hh_d45 hh_d46 if age_0_23m == 1, m 
* missing observations treated as deprived
gen     ind_birthattend = (hh_d45 != 1) & (hh_d46>2) if age_0_23m == 1
...
tab     hh_f47 if age_0_23m ==1 | age_24_59m ==1, m
* missing observations treated as deprived
gen     ind_mosqnet = hh_f47 >1 if age_0_23m ==1 | age_24_59m==1 
...
lab var dim_health "Health dimension - deprivation in at least one indicator"
********************************************************************************
...
* economic activities
codebook hh_e07a hh_e07b hh_e07c hh_e08 hh_e09 hh_e10 hh_e11 
...
egen	econ_act  =rowtotal(hh_e07a hh_e07b hh_e07c hh_e08 hh_e09 hh_e10 hh_e11)
...
lab var  dim_protection "Protection dimension - deprivation in at least one indicator"
********************************************************************************
...
* ------------------------------------------------------------------------------
*** Indicator i) Illiterate 

* Deprived if s/he cannot read and write in any language (15-17 years)
* ------------------------------------------------------------------------------

tab1 	 hh_c05_1 hh_c05_3 if age_15_17 ==1, m
* Missing observation treated as illiterate
gen	     ind_literacy = (hh_c05_1 !=1) & (hh_c05_3 !=1) if age_15_17 ==1
...
tab1     hh_c08 hh_c13 if age_15_17==1, m
...
lab var dim_education "Education dimension - deprivation in at least one indicator"
********************************************************************************
...
* mobile phone, PC, or satelite dish
* ------------------------------------------------------------------------------

gen      ind_information = (inforasset==0) & (hh_b06_4==2) & (hh_f31==2)    ///
             if age_5_17==1
...
lab var dim_information "Information dimension - deprivation in at least one indicator"
********************************************************************************
...
gen      ind_watersource = inlist(hh_f36, 4,5,10,11,12,15,16,18) if hh_b05a <18
replace  ind_watersource = 1 if inlist(hh_f40, 4,5,10,15)
lab var  ind_watersource   "Unimproved drinking water (0-17 years)" 
             
* ------------------------------------------------------------------------------

*** Indicator ii) 30 minutes to water
...
lab var dim_water "Water dimension - deprivation in at least one indicator"
********************************************************************************
...
gen      ind_sanitation = inlist(hh_f41, 4,8,10,11,12,13,.) if hh_b05a <18
...
gen      ind_roofflow = inlist(hh_f08, 1,5,6) & inlist(hh_f09, 1,2,6)        ///
             if hh_b05a <18
...
qui     ds(interview_status qx_type consent interviewdate_v1 haz06           ///
         interviewdate_v2 hh_* hhsize agemonths measure inforasset region_   ///
         id_code watermin econ_act hhchore waz06 whz06 bmiz06 age_* chore_a  ///
		 chore_b *_labor_* labor_15_17), not         
		 
keep  `r(varlist)'

  keep    if flag_0_17 == 1
  drop    flag_0_17
  save    "${pathout}/cmdpi-2019.dta", replace    	
...
egen      msize = total(male), by(y4_hhid)
label var msize "Number of males in hh"

egen fsize = total(female), by(y4_hhid)
...
tab1 	  hh_c05_1 hh_c05_3
gen       literateHoh    = (hh_c05_1==1) & (hh_c05_3 == 1) if hoh    == 1
gen       literateSpouse = (hh_c05_1==1) & (hh_c05_3 == 1) if hh_b04 == 2
...
* Malawi definition of youth is 10-35 years (Nationa Youth Policy (2023-2028))
...
foreach x of local demo {
    egen `x'  = total(`x'tmp), by(y4_hhid)
    egen `x'm = total(`x'tmp) if male   == 1, by(y4_hhid)
    egen `x'f = total(`x'tmp) if female == 1, by(y4_hhid)
...
egen totNumDepRatio   = total(numDepRatio),   by(y4_hhid)
egen totDenomDepRatio = total(demonDepRatio), by(y4_hhid)
...
egen      femCount20_34 = total(fem20_34tmp), by(y4_hhid)
egen      femCount35_59 = total(fem35_59tmp), by(y4_hhid)
...
egen      adultEquiv = total(ae), by(y4_hhid)
...
********************************************************************************
gen       orphan   = (hh_b16a==98) | (hh_b19a==98) if hh_b05a < 18
egen      orphanhh = total(orphan), by(y4_hhid)
...
egen educAdult  = max(educ) if hh_b05a>17, by(y4_hhid)
egen educAdultM = max(educ) if hh_b05a>17 & male   == 1, by(y4_hhid)
egen educAdultF = max(educ) if hh_b05a>17 & female == 1, by(y4_hhid)

* Max youth education in hh
egen educYouth  = max(educ) if youth10to35tmp == 1, by(y4_hhid)
egen educYouthM = max(educ) if youth10to35tmp == 1 & male   == 1, by(y4_hhid)
egen educYouthF = max(educ) if youth10to35tmp == 1 & female == 1, by(y4_hhid)
...
save "${pathout}/hhold-char-ind-2019.dta", replace    	
...
qui ds(y4_hhid y3_hhid case_id HHID ea_id PID hhmember baselinemember        ///
	  baselinemembercount individualmover interviewdate_v1 interviewdate_v2  ///
	  consumption_date moverbasehh), not

collapse (firstnm) HHID y3_hhid case_id ea_id (max) `r(varlist)', by(y4_hhid) 
...
foreach x of varlist youth* under* child0* {
	replace `x' = 0 if `x' == .
}
...
lab var age_head    "Age of household head (years)"
lab var male_head   "1 = Male head of househod"
lab var literateHoh "1 = Literate head of household"
lab var mstatus1    "1 = Monogamous head of household"	
lab var mstatus2    "1 = Polygamous head of household"	
lab var mstatus3    "1 = Separated/divorced head of household"	
lab var mstatus4    "1 = Didow(er) head of household"	
lab var mstatus5    "1 = Never married head of household"	
lab var marriedhead "1 = Married head of household"
		
save "${pathout}/hhold-char-2019.dta", replace

log2html "$pathlog/01_cmdpi_2019", replace
log close
