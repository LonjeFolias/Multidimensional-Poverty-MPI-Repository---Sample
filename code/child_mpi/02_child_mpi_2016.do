...
*******************************************************************************
    ...
    log   using  "$pathlog/01_cmdpi_2016", replace
	
    di in yellow "`c(current_date)' `c(current_time)'"
	
/*******************************************************************************
# Name:			01_Child-Multi-Dimensional Poverty Index 2016

# Purpose:		Generates Child Multi-Dimensional Poverty Index for 2016
...
*	Uses the following modules;
		(01) hh_mod_a_filt_16 - Household Identification
		(02) hh_mod_b_16	  - Household Roster
		(03) hh_mod_c_16	  - Education
		(04) hh_mod_d_16	  - Health
		(05) hh_mod_e_16	  - Time Use and Labour
		(06) hh_mod_f_16	  - Housing
		(07) hh_mod_h_16	  - Food Security
		(08) hh_mod_v_16	  - Child Anthropometry
		(09) hh_mod_l_16	  - Durable goods
		(10) 
*******************************************************************************/
...
use      "${wave2}/hh_mod_l_16.dta", clear  
...
collapse (max) inforasset, by(y3_hhid)
lab var  inforasset "HH has any of the information assets: radio, TV, PC, satelite dish"

* Merge Household Identification
merge    1:1 y3_hhid   using "${wave2}/hh_mod_a_filt_16.dta", nogen 
 
* Merge Household Roster
merge    1:m y3_hhid   using "${wave2}/hh_mod_b_16.dta", nogen  

* Merge Education
merge 1:1  y3_hhid PID using "${wave2}/hh_mod_c_16.dta", keep (3) nogen 

* Merge Health
merge 1:1  y3_hhid PID using "${wave2}/hh_mod_d_16.dta", keep (3) nogen 

* Merge Time Use and Labour
merge 1:1  y3_hhid PID using "${wave2}/hh_mod_e_16.dta", keep (3) nogen

* Merge Housing
merge m:1  y3_hhid     using "${wave2}/hh_mod_f_16.dta", keep (3) nogen

* Merge Food Security
merge m:1 y3_hhid  	   using "${wave2}/hh_mod_h_16.dta", keep (3) nogen

* Merge Child Anthropometry
merge 1:1  y3_hhid PID using "${wave2}/hh_mod_v_16.dta", keep (3) nogen
...
global   id_weights y3_hhid HHID case_id panelweight_2016 panelweight_2013 hh_wgt region reside district 
...
* Note: There is Chichewa and English only in 2016 wave
* ------------------------------------------------------------------------------
...
/*	8-4-4 system (https://www.scholaro.com/db/Countries/Malawi/Education-System)
...
codebook inforasset hh_b04a hh_f31
gen      ind_information = (inforasset==0) & (hh_b04a==2) & (hh_f31==2) ///
		     if age_5_17==1
...
gen     ind_watersource =inlist(hh_f36,4,5,10,11,12,14,16)     if hh_b05a <18
replace ind_watersource =1 if inlist(hh_f36,4,5,10,11,12,14,16) & hh_b05a <18
...
qui     ds(reside interview_status qx_type consent interviewdate_v1 region_  ///
           interviewdate_v2 hh_* hhsize agemonths measure inforasset haz06   ///
           id_code watermin econ_act hhchore waz06 whz06 bmiz06 age_*        ///
           labor_15_17 *_labor_* ), not
		 
keep  `r(varlist)'

  keep    if flag_0_17 == 1
  drop    flag_0_17
  save    "${pathout}/cmdpi-2016.dta", replace    	
...
egen      msize = total(male), by(y3_hhid)
label var msize "Number of males in hh"

egen fsize = total(female), by(y3_hhid)
...
foreach x of local demo {
    egen `x'  = total(`x'tmp), by(y3_hhid)
    egen `x'm = total(`x'tmp) if male   == 1, by(y3_hhid)
    egen `x'f = total(`x'tmp) if female == 1, by(y3_hhid)
...
egen totNumDepRatio   = total(numDepRatio),   by(y3_hhid)
egen totDenomDepRatio = total(demonDepRatio), by(y3_hhid)
...
egen      femCount20_34 = total(fem20_34tmp), by(y3_hhid)
egen      femCount35_59 = total(fem35_59tmp), by(y3_hhid)
...
egen      adultEquiv = total(ae), by(y3_hhid)
...
egen      orphanhh = total(orphan), by(y3_hhid)
...
egen educAdult  = max(educ) if hh_b05a>17, by(y3_hhid)
egen educAdultM = max(educ) if hh_b05a>17 & male   == 1, by(y3_hhid)
egen educAdultF = max(educ) if hh_b05a>17 & female == 1, by(y3_hhid)

* Max youth education in hh
egen educYouth  = max(educ) if youth10to35tmp == 1, by(y3_hhid)
egen educYouthM = max(educ) if youth10to35tmp == 1 & male   == 1, by(y3_hhid)
egen educYouthF = max(educ) if youth10to35tmp == 1 & female == 1, by(y3_hhid)
...
save "${pathout}/hhold-char-ind-2016.dta", replace    	

* Collapse everything down to HH-level using max values for all vars
* Copy variable labels to reapply after collapse
qui include "$pathdo/copylabels.do"

qui ds(y3_hhid case_id HHID ea_id PID hhmember baselinemember               ///
    baselinemembercount individualmover interviewdate_v1 interviewdate_v2   ///
    consumption_date moverbasehh y2_hhid i_*), not

collapse (firstnm) HHID case_id y2_hhid ea_id (max) `r(varlist)', by(y3_hhid) 
...
save "${pathout}/hhold-char-2016.dta", replace 

log2html "$pathlog/01_cmdpi_2016", replace
log close
